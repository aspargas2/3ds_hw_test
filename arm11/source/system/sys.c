...
void SYS_CoreZeroInit(void)
{
	...
	*LEGACY_BOOT_ENTRYPOINT = 0;
	...
	// Map all sections here, located in VRAM
	...
	// BootROM
	mmuMapArea(0x17000000, 0x00010000, 32UL << 10, MMU_FLAGS(MMU_CACHE_WT, MMU_READ_ONLY, 0, 1));
	...
	// Start of VRAM, for framebuffers
	mmuMapArea(0x18000000, 0x18000000, 1UL << 20, MMU_FLAGS(MMU_CACHE_WT, MMU_READ_WRITE, 1, 1));

	// DSP Memory + AXIWRAM
	mmuMapArea(0x1FF00000, 0x1FF00000, 1UL << 20, MMU_FLAGS(MMU_STRONG_ORDER, MMU_READ_WRITE, 1, 1));

	// FCRAM
	if (SYS_IsNewConsole()) {
		mmuMapArea(0x20000000, 0x20000000, 256UL << 20, MMU_FLAGS(MMU_STRONG_ORDER, MMU_READ_WRITE, 1, 1));
	} else {
		mmuMapArea(0x20000000, 0x20000000, 128UL << 20, MMU_FLAGS(MMU_STRONG_ORDER, MMU_READ_WRITE, 1, 1));
	}
